...
只输出恰好 ${lineCount} 句的 JSON 数组：[{"说话人":"简称","台词":"……"}]`;
  ...
  function formatStates(c) {
    ...
    return c.activeStates.map(st => {
      ...
      const time = st.remaining >= 999990 ? '（长久）'
  ...
  function formatMood(c) {
    ...
    const low = getNeedDefs()
      ...
      .filter(n => n.val < 30)
      .map(n => n.label + '不足');
    if (avg >= 70 && !low.length) return '心气尚可';
...
只输出恰好 ${lineCount} 句的 JSON 数组：[{"说话人":"简称","台词":"……"}]`;
  ...
  function renderHudRelBar(label, val, min, max, color, base) {
    ...
    return `<div class="ldo-bar-row">
      <span class="ldo-bar-label">${escapeHtml(label)}</span>
  ...
  function renderHudScoreBar(score, base) {
    const pct = Math.max(0, Math.min(100, (score + 100) / 2));
    const d = Math.round(score - base);
    const delta = d ? `<span class="ldo-delta ${d > 0 ? 'up' : 'down'}">${d > 0 ? '+' : ''}${d}</span>` : '';
    const scoreStr = (score >= 0 ? '+' : '') + score;
    return `<div class="ldo-bar-row ldo-bar-score">
      <span class="ldo-bar-label">情分</span>
      <div class="ldo-bar-track"><div class="ldo-bar-fill" style="width:${pct}%;background:#c9a227"></div></div>
  ...
  function renderParticipantsHud() {
    ...
    const axisHtml = [
      renderHudRelBar('友爱', ri.affection, -100, 100, ri.affection >= 0 ? '#c0392b' : '#7f8c8d', base.affection),
      renderHudRelBar('信任', ri.trust, -100, 100, ri.trust >= 0 ? '#2980b9' : '#7f8c8d', base.trust),
      renderHudRelBar('情谊', ri.friendship, 0, 100, '#27ae60', base.friendship),
      renderHudRelBar(`${a.short}服${b.short}`, ri.submissionAtoB, 0, 100, '#9b59b6', base.submissionAtoB),
      ri.submissionBtoA > 5
        ? renderHudRelBar(`${b.short}服${a.short}`, ri.submissionBtoA, 0, 100, '#8e44ad', base.submissionBtoA)
        : '',
    ].join('');

    el.innerHTML = `
      <div class="ldo-char${speakingId === a.id ? ' speaking' : ''}">
        <div class="ldo-av" style="background:${a.color}"></div>
      ...
      <div class="ldo-rel-center">
        ${renderHudScoreBar(ri.score, base.score)}
        <div class="ldo-rel-type">${escapeHtml(ri.typeLabel || '')}${ri.initType ? ` · ${escapeHtml(ri.initType)}` : ''}</div>
        <div class="ldo-rel-bars">${axisHtml}</div>
      </div>
      <div class="ldo-char${speakingId === b.id ? ' speaking' : ''}">
        <div class="ldo-av" style="background:${b.color}"></div>
  ...
  return {
    ...
    shouldUse, shouldUseAwkward, isEnabled, loadPrompts, isPending, requestSwitchInteraction,
  };
})();
